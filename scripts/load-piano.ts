...
async function main(): Promise<void> {
  ...
  try {
    ...
    const groupState = await admin.waitForState(
      (state) => state.phase === "phase2_groups",
      10_000,
      "phase2_groups"
    );

    const assignedTarget = assignStudentsToPianoGroups(students, groupState);
    await admin.waitForState(
      (state) => countPianoAssignments(state) >= assignedTarget,
      10_000,
      "piano group assignments"
    );

    const assignmentState = admin.state ?? groupState;
    const activePlayers = activePianoPlayers(students, assignmentState);
    console.info(
      `Piano players active: ${activePlayers.length}; waiting players: ${
        students.length - activePlayers.length
      }`
    );

    if (activePlayers.length === 0) {
      throw new Error("No student could be assigned to a piano group.");
    }

    admin.send({ type: "admin_advance_phase" });
    await admin.waitForState(
      (state) => state.phase === "phase3_practice",
      10_000,
      "phase3_practice"
    );

    console.info("Starting piano performance...");
    const performanceStartPromise = admin.waitFor(
      (message): message is Extract<ServerMessage, { type: "performance_start" }> =>
        message.type === "performance_start" && message.scoreId === "piano_only",
      10_000,
      "piano performance_start"
    );
    admin.send({ type: "admin_advance_phase" });
    ...
    const performanceState = await admin.waitForState(
      (state) =>
        ...
        state.currentScoreId === "piano_only" &&
        Boolean(state.performanceStartAtServerMs),
      10_000,
      "phase4_performance"
    );

    const score = performanceState.scores.piano_only;
...
function printUsage(): void {
  console.info(`
Usage:
  bun run load:piano -- --api http://localhost:3000 --admin-password admin123 --players 50

Options:
  --api                 API base URL or websocket URL. Default: ${DEFAULTS.api}
  --admin-password      Admin password. Default: ADMIN_PASSWORD or admin123
  --players             Number of student connections. Default: 50
...
function assignStudentsToPianoGroups(
  students: RealtimeClient[],
  state: PublicState
): number {
  const rooms = state.parts.filter((room) => room.scoreId === "piano_only");
  const slots: string[] = [];
...
function countPianoAssignments(state: PublicState): number {
  return state.parts
    .filter((room) => room.scoreId === "piano_only")
    .reduce((sum, room) => sum + room.students.length, 0);
}

function activePianoPlayers(
  students: RealtimeClient[],
  state: PublicState
): Array<{ client: RealtimeClient; part: ScorePart }> {
  const score = state.scores.piano_only;
...
function printSummary(input: {
  ...
  console.info(`Piano players active: ${input.activePlayers}`);
